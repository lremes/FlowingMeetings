if defined?(Rails::Server)
	HealthCheck.setup do |config|
		...
		config.full_checks = ['database', 'migrations', 'custom', 'email', 'cache', 'redis' ]
		...
		#config.basic_auth_username = ENV.fetch('LAWLY_HEALTH_CHECK_USERNAME', Rails.application.credentials.dig(:health_check,:username))
		#config.basic_auth_password = ENV.fetch('LAWLY_HEALTH_CHECK_PASSWORD', Rails.application.credentials.dig(:health_check,:password))
